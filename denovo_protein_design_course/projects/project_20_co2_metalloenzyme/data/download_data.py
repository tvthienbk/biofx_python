...
# ---------------------------------------------------------------------------
# EDIT ONLY THIS for each project. (Project 20 — CO2-Fixing Metalloenzyme,
# carbonic-anhydrase-style Zn-His3-OH site.)
# These are candidate REFERENCE carbonic anhydrase II structures used to read off the real
# Zn-His3 site geometry and as a natural-CA positive control for the assay logic — NOT the
# theozyme/metal site itself (you CONSTRUCT that into data/inputs/metal_site_def.txt). EVERY
# accession below is a CANDIDATE — verify it on RCSB in Week 1 before use. PDB entries get
# superseded; these IDs are flagged "look up + verify" rather than asserted. If you cannot confirm
# an ID, comment it out and keep the verify note (or set ACCESSIONS = []).
# ---------------------------------------------------------------------------
ACCESSIONS: list[Item] = [
    # Candidate reference human carbonic anhydrase II structures (read off the Zn-His3 geometry;
    # use as the natural-CA positive control). VERIFY each ID + chain on RCSB before relying on it:
    Item("2CAB", "rcsb", note="CANDIDATE — verify on RCSB: a human carbonic anhydrase II structure "
                              "with the catalytic Zn-His3 site; read off Zn-N distances / N-Zn-N "
                              "angles. Confirm the exact ID/variant + chain first."),
    Item("3KS3", "rcsb", note="CANDIDATE — verify on RCSB: a second human carbonic anhydrase II "
                              "candidate (cross-check the Zn-His3 geometry against 2CAB). Confirm "
                              "before use."),
    # If neither verifies, fall back to an empty list and rely on the constructed metal-site template:
    #   ACCESSIONS = []   # and record in LOG.md that no reference CA PDB could be verified.
    #
    # NOTE: the Zn-His3-OH metal site geometry is NOT a single fetch — you read it from a verified CA
    # structure and encode it into data/inputs/metal_site_def.txt (a template to fill). The GRACE
    # de-novo CA designs (Hu 2024) and their reported activity live in that paper's supplementary
    # data, not a single-accession pull — log their DOI/table/license in data/README.md.
]
