...
# ---------------------------------------------------------------------------
# EDIT ONLY THIS for each project. These are the Project 10 inputs: NDM-1
# (New Delhi metallo-beta-lactamase) structures with the DI-ZINC active site.
# Treat each accession as "candidate — verify on RCSB in Week 1" (the catalog
# flags these may be superseded). CRITICAL di-zinc-site note: NDM-1 is a class B
# metallo-beta-lactamase whose active site contains TWO catalytic Zn2+ ions
# (Zn1: 3 His; Zn2: Asp/Cys/His). In target prep you MUST preserve both Zn2+ ions
# (keep them as heteroatoms) — they are part of the epitope, and the binder sits on
# the active-site RIM to OCCLUDE substrate (carbapenem) access. Do NOT strip the
# metals or design over the Zn-coordinating ligand residues. See data/README.md.
# ---------------------------------------------------------------------------
ACCESSIONS: list[Item] = [
    # NDM-1 di-zinc metallo-beta-lactamase structures — CANDIDATES, verify on RCSB in Week 1.
    Item("3SPU", "rcsb", note="candidate NDM-1 with di-zinc active site — primary target prep (preserve both Zn2+); read off the active-site-RIM hotspots here"),
    Item("4EYL", "rcsb", note="candidate NDM-1 (often with a hydrolyzed beta-lactam) — cross-check the di-zinc geometry + substrate-channel rim against 3SPU"),
    # Optional: pull the NDM-1 (blaNDM-1) protein sequence for residue-numbering reference (verify accession).
    # Item("C7C422", "uniprot", note="NDM-1 (blaNDM-1) sequence (verify accession) — domain/residue numbering reference"),
    # Optional human off-target metalloenzyme reference for the specificity counter-test (notebook 04 / control in 05):
    # Item("1CA2", "rcsb", note="human carbonic anhydrase II (Zn) — off-target SPECIFICITY counter-test reference (verify)"),
]
