...
# ---------------------------------------------------------------------------
# EDIT ONLY THIS for each project. (Project 21 — De Novo Serine Hydrolase / Esterase,
# Green Chemistry.) These are REFERENCE serine hydrolases used as structural references,
# triad-geometry sanity checks, and positive controls — NOT the substrate or theozyme
# (you CONSTRUCT those: the Ser-His-Asp triad + oxyanion hole + ester/tetrahedral-
# intermediate transition state; see data/inputs/theozyme_def.txt). EVERY accession below
# is a CANDIDATE — verify it on RCSB in Week 1 before use. Serine-hydrolase entries are
# numerous (wild-type, mutants, complexes, and the new de novo Lauko-2025 designs) and get
# superseded; these IDs are flagged "look up + verify" rather than asserted. If you cannot
# confirm an ID, comment it out and keep the verify note rather than fetching the wrong
# structure (or set ACCESSIONS = []).
# ---------------------------------------------------------------------------
ACCESSIONS: list[Item] = [
    # Candidate reference serine hydrolases (structural references + triad-geometry references +
    # positive controls). VERIFY each ID + chain on RCSB before relying on it:
    Item("1CEX", "rcsb", note="CANDIDATE — verify on RCSB: a Fusarium solani cutinase candidate — a "
                              "compact alpha/beta-hydrolase with the classic Ser-His-Asp triad + oxyanion "
                              "hole; a clean triad-geometry reference for a small esterase. Confirm ID/chain."),
    Item("3TGL", "rcsb", note="CANDIDATE — verify on RCSB: a Rhizomucor miehei lipase candidate — a "
                              "well-studied serine-hydrolase triad with a lid/larger acyl pocket (longer-acyl "
                              "substrate-scope reference). Confirm ID/variant; many lipase entries exist."),
    # Also worth looking up + verifying as a frontier comparator: a Lauko-2025 (Science) DE NOVO
    # serine-hydrolase design entry — the result this project reproduces — IF a coordinate file has
    # been deposited/released. Treat its accession as a candidate to look up on RCSB; do not assert one
    # here. If neither candidate above verifies, fall back to an empty list and rely on the constructed
    # theozyme only:
    #   ACCESSIONS = []   # and record in LOG.md that no reference PDB could be verified.
    #
    # NOTE: the chromogenic-ESTER substrate (p-nitrophenyl acetate/butyrate) + the tetrahedral-
    # intermediate transition-state geometry are NOT fetched here — you build them from the serine-
    # hydrolase mechanism / QM into data/inputs/theozyme_def.txt (a template to fill). Designed/natural-
    # enzyme SEQUENCES with reported kinetics live in paper supplementary tables, not a single-accession
    # fetch — log their DOI/table/license in data/README.md.
]
