...
# ---------------------------------------------------------------------------
# EDIT ONLY THIS for each project. (Project 18 — De Novo Kemp Eliminase.)
# These are REFERENCE designed Kemp eliminases used as positive controls / catalytic-
# geometry sanity checks — NOT the substrate or theozyme (you CONSTRUCT those; see
# data/inputs/theozyme_def.txt). EVERY accession below is a CANDIDATE — verify it on
# RCSB in Week 1 before use. PDB entries get superseded, and these IDs are flagged
# "look up + verify" rather than asserted. If you cannot confirm an ID, comment it out
# and keep the verify note rather than fetching the wrong structure (or set ACCESSIONS = []).
# ---------------------------------------------------------------------------
ACCESSIONS: list[Item] = [
    # Candidate reference designed Kemp eliminases (positive controls for the catalytic
    # geometry + assay logic). VERIFY each ID + chain on RCSB before relying on it:
    Item("3IIP", "rcsb", note="CANDIDATE — verify on RCSB: a Roethlisberger-2008 Kemp-eliminase "
                              "(KE07 lineage) candidate; confirm the exact ID/variant first."),
    Item("5D38", "rcsb", note="CANDIDATE — verify on RCSB: an HG3/HG3.17 evolved Kemp-eliminase "
                              "candidate (directed-evolution endpoint reference); confirm before use."),
    # If neither verifies, fall back to an empty list and rely on the constructed theozyme only:
    #   ACCESSIONS = []   # and record in LOG.md that no reference PDB could be verified.
    #
    # NOTE: the 5-nitrobenzisoxazole SUBSTRATE + transition-state geometry are NOT fetched here —
    # you build them from literature/QM into data/inputs/theozyme_def.txt (a template to fill).
    # Designed-enzyme SEQUENCES with reported kinetics live in paper supplementary tables, not a
    # single-accession fetch — log their DOI/table/license in data/README.md.
]
