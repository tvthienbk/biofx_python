...
# ---------------------------------------------------------------------------
# EDIT ONLY THIS for each project. (Project 19 — Plastic-Degrading Active-Site Design,
# PETase-like.) These are REFERENCE PET hydrolases / cutinases used as structural
# references, positive controls, and THERMOSTABILITY comparators — NOT the substrate or
# theozyme (you CONSTRUCT those: the Ser-His-Asp triad + oxyanion hole + ester/PET-mimic
# transition state; see data/inputs/theozyme_def.txt). EVERY accession below is a
# CANDIDATE — verify it on RCSB in Week 1 before use. PET-hydrolase / cutinase entries are
# numerous (wild-type, mutants, complexes) and get superseded; these IDs are flagged
# "look up + verify" rather than asserted. If you cannot confirm an ID, comment it out and
# keep the verify note rather than fetching the wrong structure (or set ACCESSIONS = []).
# ---------------------------------------------------------------------------
ACCESSIONS: list[Item] = [
    # Candidate reference PET hydrolases / cutinases (structural references + positive
    # controls + thermostability comparators). VERIFY each ID + chain on RCSB before use:
    Item("6EQE", "rcsb", note="CANDIDATE — verify on RCSB: an Ideonella sakaiensis IsPETase "
                              "candidate (the canonical PET hydrolase + thermostability comparator; "
                              "wild-type is fragile near PET's glass transition). Confirm ID/variant."),
    Item("5XJH", "rcsb", note="CANDIDATE — verify on RCSB: a cutinase / PET-active serine-hydrolase "
                              "candidate (Ser-His-Asp triad + oxyanion hole; engineered-natural "
                              "scaffold track + triad-geometry reference). Confirm before use."),
    # Also worth looking up + verifying (engineered THERMOSTABLE comparators for your
    # thermostability ranking): the LCC ICCG / DuraPETase lineage, and a thermophilic cutinase.
    # If neither candidate above verifies, fall back to an empty list and rely on the
    # constructed theozyme only:
    #   ACCESSIONS = []   # and record in LOG.md that no reference PDB could be verified.
    #
    # NOTE: the PET-mimic ESTER substrate + tetrahedral-intermediate transition-state geometry
    # are NOT fetched here — you build them from the serine-hydrolase mechanism / QM into
    # data/inputs/theozyme_def.txt (a template to fill). Engineered-enzyme SEQUENCES with
    # reported kinetics/Tm live in paper supplementary tables, not a single-accession fetch —
    # log their DOI/table/license in data/README.md.
]
