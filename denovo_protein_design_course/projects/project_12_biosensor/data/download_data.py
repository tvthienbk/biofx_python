...
# ---------------------------------------------------------------------------
# EDIT ONLY THIS for each project. These are the Project 12 inputs (biosensor:
# binder -> a chosen biomarker analyte). The ANALYTE IS THE STUDENT'S CHOICE
# (e.g. a cytokine or a cardiac marker) — the accession below is only an EXAMPLE
# so the plumbing has something to fetch. Treat every accession as "candidate —
# verify on RCSB/UniProt in Week 1" (entries are occasionally superseded), and
# REPLACE the example with your chosen analyte. The binder EPITOPE/HOTSPOTS come
# from the analyte surface: isolate the analyte chain, remove waters/heteroatoms/
# irrelevant partners, and list the surface residues you will target as your design
# hotspots. The SWITCH / split-reporter reference designs are literature-driven
# (Langan 2019 LOCKR; Quijano-Rubio 2021; Dixon 2016 NanoBiT) — there is no single
# download; record their DOIs. See data/README.md.
# ---------------------------------------------------------------------------
ACCESSIONS: list[Item] = [
    # EXAMPLE biomarker analyte — CANDIDATE, verify on RCSB in Week 1, or REPLACE with your choice.
    Item("1J1E", "rcsb", note="candidate EXAMPLE cardiac biomarker (human cardiac troponin core) — verify on RCSB or replace with your chosen analyte (e.g. a cytokine such as IL-6/TNF-a)"),
    # Optional off-target analyte for the specificity control (the sensor must NOT switch for it):
    # Item("<PDBID>", "rcsb", note="off-target analyte (verify) — specificity control"),
    # Optional: pull the analyte sequence (UniProt) for reference / numbering.
    # Item("<UNIPROT>", "uniprot", note="analyte sequence (verify) — numbering reference"),
]
