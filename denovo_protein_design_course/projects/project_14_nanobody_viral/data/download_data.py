...
# ---------------------------------------------------------------------------
# EDIT ONLY THIS for each project. Project 14 = conserved-epitope neutralizing nanobody;
...
ACCESSIONS: list[Item] = [
    # Project 14 inputs. DEFENSIVE design: a CONSERVED NEUTRALIZING viral epitope to BLOCK.
    # Candidates — VERIFY on RCSB in Week 1 (entries get superseded).
    Item("4FQI", "rcsb", note="Influenza HA + broadly-neutralizing Ab; defines the conserved HA-STEM epitope"),
    # Alternative target: RSV F glycoprotein in the prefusion state (neutralization-sensitive), e.g.:
    # Item("5UDE", "rcsb", note="RSV F prefusion (DS-Cav1) — alternative conserved neutralizing target — VERIFY"),
    # For the cross-strain BREADTH analysis, assemble a panel of HA (or RSV F) sequences across
    # subtypes/strains and align them to score epitope conservation (sequence pulls the student adds).
]
