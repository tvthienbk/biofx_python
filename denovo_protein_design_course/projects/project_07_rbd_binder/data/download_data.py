...
# ---------------------------------------------------------------------------
# EDIT ONLY THIS for each project. Project 07 = conserved-epitope RBD neutralizing binder;
# replace with your project's accessions. Treat each accession as "candidate —
# verify on RCSB/UniProt in Week 1" (the catalog flags which may be superseded).
# ---------------------------------------------------------------------------
ACCESSIONS: list[Item] = [
    # Project 07 inputs. DEFENSIVE design: define the conserved ACE2-binding epitope to BLOCK.
    # Candidates — VERIFY on RCSB in Week 1 (entries get superseded).
    Item("6M0J", "rcsb", note="SARS-CoV-2 RBD-ACE2 complex; defines the ACE2-binding (neutralizing) face"),
    # For the cross-variant BREADTH analysis, the student assembles a panel of RBD sequences/structures
    # (Wuhan/Delta/Omicron sublineages + other sarbecoviruses) and aligns them to score epitope
    # conservation. These are sequence pulls / additional complexes the student verifies and adds, e.g.:
    # Item("7T9K", "rcsb", note="Omicron RBD-ACE2 (variant breadth) — VERIFY"),
    # Item("P0DTC2", "uniprot", note="SARS-CoV-2 spike (S) UniProt; slice the RBD for the variant panel"),
]
