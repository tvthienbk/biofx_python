...
# ---------------------------------------------------------------------------
# EDIT ONLY THIS for each project. Project 03 (RFdiffusion monomer) needs NO external
# data to *generate* backbones — RFdiffusion designs from noise. These few small natural
# reference folds are only for novelty-scoring sanity checks (a natural fold should score
# high against the PDB). Treat each accession as "candidate — verify on RCSB in Week 1"
# (PDB entries are occasionally superseded). They span the α / β / mixed topologies the
# campaign generates, so they also calibrate the novelty axis.
#
# NOTE: the Foldseek PDB/AFDB database used to actually SCORE novelty is LARGE and is
# downloaded SEPARATELY (see data/README.md: `foldseek databases PDB ...`). It is NOT
# fetched here and must NEVER be committed to git.
# ---------------------------------------------------------------------------
ACCESSIONS: list[Item] = [
    Item("1UBQ", "rcsb", note="Ubiquitin — small robust alpha/beta fold (novelty sanity ref)"),
    Item("1L2Y", "rcsb", note="Trp-cage — tiny all-alpha fast-folder (alpha topology ref)"),
    Item("2GB1", "rcsb", note="Protein G B1 domain — classic alpha/beta (mixed topology ref)"),
    Item("1ENH", "rcsb", note="Engrailed homeodomain — small all-alpha"),
    Item("1MJC", "rcsb", note="Cold-shock protein — beta-barrel (all-beta topology ref)"),
]
