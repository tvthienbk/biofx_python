...
# ---------------------------------------------------------------------------
# EDIT ONLY THIS for the Project 25 capstone. This script fetches YOUR chosen,
# advisor-approved TARGET structure/sequence — replace the placeholder below with
# your target's accession(s). Treat each as "candidate — verify on RCSB/UniProt in
# Week 1" (entries get superseded), and confirm the framing is in-scope under
# MASTER_BLUEPRINT.md §7 (advisor must approve BEFORE the design campaign / P2).
#
# The capstone's OTHER input — the cohort design->outcome feature table — is NOT
# fetched here: you ASSEMBLE it from the Projects 01-24 outputs (each project's
# filtered design table: sequences + in-silico metrics + any experimental labels),
# normalize the columns to ml_predictor.FEATURE_COLUMNS, and write
# data/cohort_design_outcomes.csv. With no real cohort yet, ml_predictor.py
# generates a clearly-labeled EXAMPLE_DATA synthetic cohort so notebook 04 runs
# anywhere. See data/README.md.
# ---------------------------------------------------------------------------
ACCESSIONS: list[Item] = [
    # PLACEHOLDER — replace with YOUR advisor-approved target. CANDIDATE, verify on RCSB in Week 1.
    Item("1UBQ", "rcsb", note="placeholder small natural fold — REPLACE with your advisor-approved target (verify; §7 in-scope)"),
    # e.g. Item("<YOUR_PDB>", "rcsb", note="your target complex/structure — verify chains, resolution, framing"),
    # e.g. Item("<YOUR_UNIPROT>", "uniprot", note="your target sequence for numbering reference — verify"),
]
