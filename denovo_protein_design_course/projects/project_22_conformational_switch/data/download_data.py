...
# ---------------------------------------------------------------------------
# EDIT ONLY THIS for each project. Project 22 (Conformational-Switch / Multi-State Protein)
# needs, as starting points, a few REFERENCE switch / two-state designs (LOCKR, hinge / open-
# closed pairs) so you can (a) see what a real designed switch looks like, and (b) define your
# own two target-state backbones. There is NO single canonical accession for "a switch", so the
# entries below are CANDIDATES flagged "candidate — verify on RCSB" — confirm each on RCSB
# before use (PDB entries are occasionally superseded, and switch designs are deposited under
# varied names). If a candidate does not resolve, use the look-up guidance in data/README.md to
# find the current accession for LOCKR / hinge / two-state designs, and log what you used.
#
# NOTE: the two-state DEFINITION you actually design against (state A / state B + the trigger)
# is a small text file you author by hand — see data/inputs/two_state_def.txt (template). The
# two RFdiffusion backbones for state A and state B are GENERATED in notebook 02, not downloaded.
# ---------------------------------------------------------------------------
ACCESSIONS: list[Item] = [
    # --- Reference switch / two-state DESIGNS (verify each on RCSB; these are candidates) -------
    Item("6MSP", "rcsb", note="candidate — verify on RCSB: LOCKR-family de novo switch (Langan 2019). "
                              "Confirm the exact LOCKR/Co-LOCKR accession on RCSB before use."),
    Item("6X8N", "rcsb", note="candidate — verify on RCSB: designed two-state / hinge-like protein "
                              "(open vs closed). Re-check on RCSB; substitute the current accession if superseded."),
    # --- Natural conformational-change references (well-characterized switches) -----------------
    Item("1AKE", "rcsb", note="candidate — verify on RCSB: adenylate kinase OPEN state — classic "
                              "natural hinge / two-state reference for what a real switch looks like."),
    Item("4AKE", "rcsb", note="candidate — verify on RCSB: adenylate kinase CLOSED state — the partner "
                              "of 1AKE; the open/closed pair is a textbook two-state hinge example."),
    Item("2LV1", "rcsb", note="candidate — verify on RCSB: a LOV-domain / photoswitch reference for the "
                              "light-switch (LOV) STRETCH task. Confirm a current LOV accession on RCSB."),
]
