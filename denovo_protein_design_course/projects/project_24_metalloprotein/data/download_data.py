...
# ---------------------------------------------------------------------------
# EDIT ONLY THIS for each project. (Project 24 — Metalloprotein / Cofactor-Binding.)
# These are REFERENCE cofactor-binding proteins used to READ OFF the target coordination
# geometry and as positive controls / sanity checks — NOT the cofactor itself (you CONSTRUCT
# the cofactor-site spec; see data/inputs/cofactor_site_def.txt). EVERY accession below is a
# CANDIDATE — verify it (and the exact chain/cofactor) on RCSB in Week 1 before use. PDB
# entries get superseded, and these IDs are flagged "look up + verify" rather than asserted.
# If you cannot confirm an ID, comment it out and keep the verify note rather than fetching the
# wrong structure (or set ACCESSIONS = [] and rely on the constructed cofactor-site spec only).
# ---------------------------------------------------------------------------
ACCESSIONS: list[Item] = [
    # Heme reference (read off the Fe coordination + axial-ligand geometry; positive control):
    Item("1MBN", "rcsb", note="CANDIDATE — verify on RCSB: sperm-whale MYOGLOBIN, the classic heme "
                              "protein (proximal His F8 coordinates the Fe; distal pocket binds O2). "
                              "Use for the proximal-His O2-binding scheme + Fe-N geometry. Confirm ID."),
    # A b-type cytochrome (BIS-HIS heme) reference — the DEFAULT electron-transfer scheme. The exact
    # entry depends on which b-type cytochrome you choose; treat the ID as a CANDIDATE to look up:
    Item("3MK7", "rcsb", note="CANDIDATE — verify on RCSB: a b-type cytochrome / bis-His heme "
                              "electron-transfer reference (axial His/His). Confirm this is a "
                              "bis-His b-type heme before relying on it; otherwise substitute another "
                              "verified b-type cytochrome (e.g. a cytochrome b562 entry)."),
    # [4Fe-4S] ferredoxin reference (read off the Fe-S(Cys) cubane geometry + the CxxCxxC...C motif):
    Item("1FXD", "rcsb", note="CANDIDATE — verify on RCSB: a ferredoxin candidate carrying an Fe-S "
                              "cluster (electron-transfer reference for the [4Fe-4S]-4Cys scheme). "
                              "Confirm the cluster type (some ferredoxins are [2Fe-2S]) before use."),
    # If none verify, fall back to an empty list and rely on the constructed cofactor-site spec only:
    #   ACCESSIONS = []   # and record in LOG.md that no reference PDB could be verified.
    #
    # NOTE: the COFACTOR (heme / [4Fe-4S] / Zn) coordination GEOMETRY is read off these references and
    # encoded into data/inputs/cofactor_site_def.txt (a template to fill) — it is not a separate fetch.
    # Designed-metalloprotein SEQUENCES (DeGrado/Baker maquettes) live in paper supplementary tables,
    # not a single-accession fetch — log their DOI/table/license in data/README.md.
]
