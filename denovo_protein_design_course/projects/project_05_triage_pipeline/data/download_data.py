...
# ---------------------------------------------------------------------------
# EDIT ONLY THIS for each project. (Project 05 — Multi-Layer Triage Engine.)
#
# Project 05 scores a DESIGN POOL — and a design pool is PRODUCED BY A CAMPAIGN, not
# downloaded from a single accession. The real production pool comes from Projects 01/03
# (designs + their predictions) or from public design-paper supplementary tables; record
# each item's DOI/table/license there, not here. The teaching pool is SYNTHETIC and made
# by scripts/make_example_pool.py (clearly labeled EXAMPLE_DATA) — it is NOT downloaded.
#
# This fetcher is therefore intentionally MINIMAL: it pulls only a few natural reference
# structures so you can exercise the Cα-RMSD geometry helper (filtering_pipeline.ca_rmsd)
# and have positive controls. Treat each accession as "candidate — verify on RCSB in
# Week 1" (PDB entries are occasionally superseded).
# ---------------------------------------------------------------------------
ACCESSIONS: list[Item] = [
    Item("1UBQ", "rcsb", note="Ubiquitin — small robust alpha/beta fold; geometry-helper sanity check"),
    Item("1L2Y", "rcsb", note="Trp-cage — tiny fast-folder; positive control"),
    Item("1ENH", "rcsb", note="Engrailed homeodomain — small all-alpha; positive control"),
    # The design pool itself is NOT fetched here — it is produced by Projects 01/03 (with
    # provenance per item) or generated synthetically by scripts/make_example_pool.py.
]
