...
# ---------------------------------------------------------------------------
# EDIT ONLY THIS for each project. These are the Project 08 inputs (de novo binders
# vs KRAS). Treat each accession as "candidate — verify on RCSB in Week 1" (PDB
# entries are occasionally superseded). Two things to get right when you verify:
#   (1) NUCLEOTIDE STATE — GDP-bound ("off") vs GTP/GppNHp-bound ("on") KRAS present
#       different switch-region conformations. Pick the structure whose state matches
#       the surface you are targeting, and record it. The KRAS HOTSPOTS (switch I,
#       ~res 30-38; switch II, ~res 60-76; or an allele-specific pocket, e.g. the G12C
#       cysteine) are read off THAT structure — isolate the KRAS chain, remove
#       waters/heteroatoms (keep the bound nucleotide + Mg2+ when relevant), and list
#       the epitope residues as your design hotspots. See data/README.md.
#   (2) ISOFORM-SPECIFICITY STRUCTURES — to reason about (and later test) selectivity,
#       you also need HRAS and NRAS structures to model your binder against. The three
#       isoforms are ~90%+ identical in the G-domain and nearly identical across the
#       switch regions, so selectivity is hard; add HRAS/NRAS accessions you verify.
# ---------------------------------------------------------------------------
ACCESSIONS: list[Item] = [
    # KRAS targets — CANDIDATES, verify on RCSB in Week 1 (confirm allele, nucleotide state, chain).
    Item("4OBE", "rcsb", note="candidate WT KRAS — verify allele + nucleotide state (GDP vs GTP/analog); switch I/II hotspots come from this structure"),
    Item("6OIM", "rcsb", note="candidate KRAS G12C — verify allele/state; defines the G12C allele-specific surface for selectivity reasoning"),
    # Isoform-specificity panel — ADD verified HRAS/NRAS structures to model the binder against
    # off-target isoforms (the selectivity test). Examples below are PLACEHOLDERS — verify on RCSB.
    # Item("121P", "rcsb", note="candidate HRAS (e.g. classic G-domain structure) — VERIFY; off-target for the isoform-specificity panel"),
    # Item("5UHV", "rcsb", note="candidate NRAS — VERIFY allele/state; off-target for the isoform-specificity panel"),
    # Optional: pull the human KRAS (P01116) sequence for domain numbering / isoform alignment.
    # Item("P01116", "uniprot", note="Human KRAS sequence (verify) — G-domain numbering + KRAS/HRAS/NRAS alignment reference"),
]
