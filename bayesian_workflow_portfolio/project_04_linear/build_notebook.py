...
def broken_notebook() -> NotebookBuilder:
    ...
    nb.code(
        ...
        "# BUG 2: a prior width Normal(0, 0.5) copied from the standardized scale and\n"
        "#   applied to the RAW scale, where it means something completely different.\n"
        "#   The true raw intercept is ~2, so Normal(0, 0.5) (4 SD away) is crushingly\n"
        "#   tight and pulls alpha toward 0; the true raw slope is ~0.015, so the same\n"
        "#   width barely constrains beta. Prior widths are NOT portable across scales\n"
        "#   -- see BROKEN_BUGS.md.\n"
