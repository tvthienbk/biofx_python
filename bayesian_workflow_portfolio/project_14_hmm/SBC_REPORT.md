...
2. Simulate an HMM trajectory of length `T = 120`.
...
costs ~15–30 s, so we use `N_SIMS = 12` simulations and short trajectories. It is a
...
```
SBC over 12 simulations (T=120, draws=150/chain x2)
...
With only 12 simulations the histograms are coarse; we look for gross
