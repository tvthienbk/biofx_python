"""Recovery test for the hierarchical logistic (varying-intercepts) model.

Checks that the non-centered model recovers the population-level parameters
``mu``, ``tau``, and the global slope ``beta`` within their credible intervals.
Fast by design (light sampler, small data).
...
def test_recovers_population_params():
    ...
    # non-centered should be (near) divergence-free
