...
WEEK = {
    ...
    "notebook": [
        ...
        {"md": "### A picture first\n\n"
            "Scatter the data and overlay a separate **local** linear fit on each "
            "side of the cutoff — fit only on points within a bandwidth `h` of `c`, "
            "the same window the estimator uses (RD is a *local* comparison, never "
            "a global curve). The vertical gap between the two fitted lines *at* "
            "`c` is exactly what RD estimates."},
        {"code": "def side_fit(df, c, side, h=0.5):\n"
